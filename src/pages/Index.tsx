...
const Index = ({ department = 'all' }: IndexProps) => {
  ...
  useEffect(() => {
    ...
    if (directProductId) {
      ...
      // Expanded search criteria to include subcategory (where Admin stores types)
        ...
        <section className="products-section">
          ...
          >
            {loading ? (
              ...
              !selectedProduct && <p>No products found for this category.</p>
            ) : (
                  ...
                  <div className="product-image-wrapper">
                    <img
                      ...
                      className="w-full h-full object-cover cursor-zoom-in
                        transition-all duration-500 group-hover:scale-110 group-hover:brightness-90"
                    />
                  </div>
          ...
          <div className="product-focus-container flex flex-col gap-12 p-8 max-w-[1200px] mx-auto bg-white/50 backdrop-blur-sm rounded-3xl">
            ...
            <div className="modern-action-row">
              ...
              <div className="action-item relative">
                <button className="action-btn-outline" onClick={() => setOpenFilter(openFilter === 'Size' ? null : 'Size')}>
                  <span className="text-gray-400 mr-2">SIZE:</span> {selectedSize || '—'}
                  <ChevronDown size={14} className={`ml-auto transition-transform ${openFilter === 'Size' ? "rotate-180" : ""}`} />
                </button>
                {openFilter === 'Size' && (
                  <div className="filter-dropdown-menu">
                    {filters.Size.map((opt) => (
                      <button key={opt} onClick={() => { setSelectedSize(opt); setOpenFilter(null); }} className={`opt-btn ${selectedSize === opt ? 'active' : ''}`}>{opt}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              {/* Color Dropdown */}
              <div className="action-item relative">
                <button className="action-btn-outline" onClick={() => setOpenFilter(openFilter === 'Color' ? null : 'Color')}>
                  <span className="text-gray-400 mr-2">COLOR:</span> {selectedColor || '—'}
                  <ChevronDown size={14} className={`ml-auto transition-transform ${openFilter === 'Color' ? "rotate-180" : ""}`} />
                </button>
                {openFilter === 'Color' && (
                  <div className="filter-dropdown-menu">
                    {filters.Color.map((opt) => (
                      <button key={opt} onClick={() => { setSelectedColor(opt); setOpenFilter(null); }} className={`opt-btn ${selectedColor === opt ? 'active' : ''}`}>{opt}</button>
                    ))}
                  </div>
                )}
              </div>
